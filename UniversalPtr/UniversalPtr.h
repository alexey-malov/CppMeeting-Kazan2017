...
struct UniversalPtr
{
	...
	UniversalPtr(U& ref) : m_ptr(std::addressof(ref), &UniversalPtr::NoDelete) {}

	template<typename U, 
		typename = std::enable_if_t<std::is_convertible<U*, T*>::value>>
	UniversalPtr(U* ptr) : m_ptr(ptr, &UniversalPtr::NoDelete) {}

	template <typename U, typename D, 
		typename = std::enable_if_t<std::is_convertible<typename std::unique_ptr<U, D>::pointer, T*>::value>>
	UniversalPtr(std::unique_ptr<U, D>&& unique) : m_ptr(std::move(unique))	{}

	template <typename U, 
		typename = std::enable_if_t<std::is_convertible<U*, T*>::value>>
	UniversalPtr(const std::shared_ptr<U>& shared) noexcept : m_ptr(shared) {}
